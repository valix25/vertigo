...
namespace vertigo {
  ...
  struct SwitchVariableLinear : public gtsam::DerivedValue<SwitchVariableLinear> {
    ...
    inline gtsam::Vector localCoordinates(const SwitchVariableLinear& t2) const { return (gtsam::Vector(1) << (t2.value() - value())); }
    ...
    static inline gtsam::Vector Logmap(const SwitchVariableLinear& p) { return (gtsam::Vector(1) << p.value()); }
